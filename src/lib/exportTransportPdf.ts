...
export async function exportTransportPdf(envoi: EnvoiTransport) {
  ...
  doc.text(`BON DE TRANSPORT N°${num}`, pageW - mR - 3, 18, { align: 'right' });
  ...
  doc.text(`CH ${envoi.site_nom ?? '—'} — Laboratoire de Biologie Médicale`, mL + 3, 28);
  doc.text('Tél : 03 29 68 76 89', mL + 3, 33);
  doc.text('BON DE RÉCEPTION À RETOURNER : commandes.labo@ch-ed.fr', mL + 3, 38);

  doc.setFont('helvetica', 'bold');
  doc.setFontSize(8.5);
  doc.setTextColor(13, 148, 136);
  doc.text(`Expéditeur : CH ${envoi.site_nom ?? '—'}`, pageW / 2 + 5, 27);
  doc.text(`Destinataire : ${envoi.dest_nom ?? '—'}`, pageW / 2 + 5, 32);
  doc.setFont('helvetica', 'normal');
  doc.setTextColor(60, 60, 60);
  doc.text(`Date : ${dateStr}`, pageW / 2 + 5, 37);
  doc.text(`Opérateur : ${envoi.visa_expediteur}`, pageW / 2 + 5, 42);

  // QR code top right
  doc.addImage(qrDataUrl, 'PNG', pageW - mR - 28, 10, 25, 25);
  ...
  const rows = [
    ...
    { label: 'Congelé (≤ -15°C)', color: [237, 231, 246] as [number, number, number], items: congele },
  ];
  ...
  autoTable(doc, {
    startY: 48,
    head: [['Température', 'Nb sachets', 'Codes-barres scannés']],
    body: tableBody,
  ...
  doc.text('[ ] Ambiant : ___   [ ] +4°C : ___   [ ] Congelé : ___', mL + 3, y + 36);
  y += 50;
  ...
  doc.setFontSize(8.5);
  doc.setTextColor(60, 60, 60);
  doc.text('Date et heure de réception : ____________________________', mL + 3, y + 15);
  doc.text('Nom du réceptionnaire : _________________________________', mL + 3, y + 22);
  doc.text('Signature : _____________________________________________', mL + 3, y + 29);
  doc.text('[ ] Ambiant : ___   [ ] +4°C : ___   [ ] Congelé : ___', mL + 3, y + 36);
